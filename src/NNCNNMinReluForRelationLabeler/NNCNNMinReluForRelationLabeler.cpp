#include "NNCNNMinReluForRelationLabeler.h"
...
void Classifier::extractFeature(Feature& feat, const Instance* pInstance) {
	...
	feat.m_linear_features = pInstance->m_sparse;
	if (feat.m_linear_features.size() == 0){
		feat.m_linear_features.push_back("[s]-null-");
	}
}
